...
const EmpleadoForm = () => {
  ...
  const [mostrarFormularioEmpleado, setMostrarFormularioEmpleado] = useState(true);
  ...
  const [formData, setFormData] = useState({
    ...
    profesion: '',
    observaciones: '',
    tipo: 'administrativo' // Por defecto administrativo
  });
      ...
      const fetchEmpleado = async () => {
        try {
          ...
          setFormData({
            ...
            tipo: response.data.tipo || 'administrativo'
          });
          
          // Si estamos editando, ya tenemos un empleado creado
          setEmpleadoCreado(response.data);
          setLoading(false);
        } catch (err) {
          console.error('Error al cargar datos del empleado:', err);
          setError('Error al cargar los datos del empleado. Por favor, intente nuevamente.');
          setLoading(false);
        }
      };
  ...
  useEffect(() => {
    const cargarDocumentosRequeridos = async () => {
      try {
        ...
        const token = localStorage.getItem('token');
        const personaID = isEditing ? id : (empleadoCreado ? empleadoCreado.id : '0');
        const tipoEmpleado = formData.tipo || 'administrativo';
        
        console.log(`Cargando documentos para empleado con ID: ${personaID}, tipo: ${tipoEmpleado}`);
        
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/documentos/verificar/${personaID}/${tipoEmpleado}`,
          { headers: { 'Authorization': `Bearer ${token}` } }
        );
    ...
    if (isEditing || empleadoCreado) {
      cargarDocumentosRequeridos();
    }
  }, [id, isEditing, empleadoCreado, formData.tipo]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        // Ocultar formulario de empleado y mostrar solo documentos
        ...
        setSuccess('Empleado creado correctamente. Ahora puede subir los documentos.');
        setEmpleadoCreado(response.data);
  ...
  const handleUploadDocument = async () => {
    ...
    try {
      ...
      formData.append('personaID', isEditing ? id : empleadoCreado.id);
      ...
      try {
        const personaID = isEditing ? id : empleadoCreado.id;
        ...
        if (documentoExistente) {
          ...
          console.log('Creando nuevo documento con datos:', {
            ...
            descripcion: `Documento ${documentoSeleccionado.nombre} del empleado`
          });
        ...
        const tipoEmpleado = empleadoCreado?.tipo || 'administrativo';
        const updateResponse = await axios.get(
          `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/documentos/verificar/${isEditing ? id : empleadoCreado.id}/${tipoEmpleado}`,
          { headers: { 'Authorization': `Bearer ${token}` } }
        );
  ...
  if (loading && isEditing && !empleadoCreado) {
    return (
  ...
  return (
    <div className="min-h-screen bg-amber-50">
      <div className="container mx-auto px-4 py-8">
        {/* Header Hero Section */}
        <div className="relative overflow-hidden bg-amber-800 shadow-2xl rounded-2xl mb-8 transition-all duration-300">
          ...
          <div className="relative px-6 py-8">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div className="p-3 bg-amber-500/20 rounded-xl backdrop-blur-sm border border-amber-400/30 transition-all duration-300 hover:bg-amber-500/30">
                ...
                <div>
                  ...
                  <p className="text-amber-200">
                    {isEditing ? 'Actualiza la información del empleado' : 'Complete los datos del nuevo empleado'}
                  </p>
                </div>
              </div>
        ...
        {error && (
          <div className="mb-6 animate-fade-in-down">
            <div className="bg-red-50 border-l-4 border-red-500 rounded-r-xl shadow-lg p-4 transition-all duration-300">
        ...
        {success && (
          <div className="mb-6 animate-fade-in-down">
            <div className="bg-amber-50 border-l-4 border-amber-500 rounded-r-xl shadow-lg p-4 transition-all duration-300">
              <div className="flex items-center">
                <div className="flex-shrink-0">
                  <FaCheckCircle className="h-5 w-5 text-amber-400" />
                </div>
                <div className="ml-3">
                  <p className="text-sm font-medium text-amber-800">{success}</p>
                </div>
                <button
                  onClick={() => setSuccess('')}
                  className="ml-auto flex-shrink-0 text-amber-400 hover:text-amber-600 transition-colors duration-200"
                >
                  <FaTimes className="h-5 w-5" />
                </button>
              </div>
        ...
        {/* Formulario de datos del empleado - Solo visible si no se ha creado o estamos editando */}
            ...
            <div className="p-6">
              <form ref={formRef} onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {/* Tipo de Empleado */}
                  <div className="group">
                    <label htmlFor="tipo" className="flex items-center text-sm font-semibold text-gray-700 mb-2">
                      <FaUserCog className="mr-2 text-amber-600" />
                      Tipo de Empleado <span className="text-red-500 ml-1">*</span>
                    </label>
                    <select
                      id="tipo"
                      name="tipo"
                      ...
                      className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all duration-200 hover:border-amber-400"
                      required
                    >
                  ...
                  <div className="group">
                    <label htmlFor="genero" className="flex items-center text-sm font-semibold text-gray-700 mb-2">
                      <FaVenusMars className="mr-2 text-amber-600" />
                      Género
                    </label>
                    <select
                      id="genero"
                  ...
                  <div className="group sm:col-span-2 lg:col-span-3">
                    ...
                    <textarea
                      ...
                      onChange={handleChange}
                      className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all duration-200 hover:border-amber-400 resize-none"
                      placeholder="Información adicional sobre el empleado (horarios, responsabilidades, etc.)"
                    ></textarea>
                  </div>
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-gray-200 flex justify-end space-x-4">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="px-8 py-3 bg-amber-600 text-white font-semibold rounded-xl hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl"
                  >
                    {loading ? (
        ...
        {/* Sección de documentos (solo visible después de crear el empleado) */}
        {(isEditing || empleadoCreado) && (
            ...
            <div className="p-6">
              {loadingDocumentos ? (
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <p className="mt-4 text-amber-600 font-medium">Cargando documentos...</p>
                </div>
              ) : (
                <>
                  <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  {errorDocumentos && (
                    <div className="mt-6 bg-red-50 border-l-4 border-red-500 rounded-r-xl shadow-lg p-4 transition-all duration-300">
                      <div className="flex items-center">
                  ...
                  <div className="mt-8 pt-6 border-t border-gray-200 flex justify-end">
                    <button
                      ...
                      className="px-8 py-3 bg-amber-600 text-white font-semibold rounded-xl hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500 transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl flex items-center"
                    >
