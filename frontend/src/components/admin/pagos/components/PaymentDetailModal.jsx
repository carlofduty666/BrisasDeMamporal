    ...
    <div className="fixed inset-0 z-50 bg-black/60 overflow-y-auto animate-fadeIn" onClick={onClose}>
      <div className="min-h-full flex items-center justify-center p-4">
        <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-[85vh] animate-slideUp" onClick={(e) => e.stopPropagation()}>
          {/* Header */}
          <div className="bg-gradient-to-r from-pink-700 to-pink-800 text-white px-6 py-4 flex items-center justify-between rounded-t-2xl">
          ...
          <div className="bg-slate-50 rounded-xl p-4 md:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="p-3 rounded-lg bg-white border">
                <p className="text-xs text-slate-500 flex items-center"><FaTag className="mr-1 text-pink-700" /> Concepto</p>
                <p className="font-semibold text-slate-800">{arancel?.nombre || pago.concepto || '-'}</p>
              </div>
              <div className="p-3 rounded-lg bg-white border">
                <p className="text-xs text-slate-500 flex items-center"><FaCreditCard className="mr-1 text-pink-700" /> Método de pago</p>
                <p className="font-semibold text-slate-800">{metodo?.nombre || '-'}</p>
              </div>
              <div className="p-3 rounded-lg bg-white border">
                <p className="text-xs text-slate-500 flex items-center"><FaHashtag className="mr-1 text-pink-700" /> Referencia</p>
              ...
              <div className="p-3 rounded-lg bg-pink-50 border border-pink-200 md:col-span-3">
                <p className="text-xs text-pink-600 flex items-center"><FaMoneyBill className="mr-1" /> Total</p>
                <p className="text-xl font-extrabold text-pink-800">{formatCurrency(total)}</p>
                <p className="text-xs text-pink-700">Bs. {totalVES.toFixed(2)}</p>
              </div>
            ...
            {pago.urlComprobante && (
              <div className="mt-4">
                <h5 className="text-sm font-semibold text-slate-800 mb-2 flex items-center"><FaReceipt className="mr-2 text-pink-700" /> Comprobante</h5>
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-md bg-pink-600 text-white hover:bg-pink-700 transition-all duration-200"
                  >
